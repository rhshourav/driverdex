...
def github_pull_manifest(workspace: Path) -> None:
    ...
    with C.status("[bold bright_cyan]  Downloading manifests from GitHub …[/bold bright_cyan]",
                ...
                if r == "ok":
                    for idx in range(2, 50):
                        ...
                        if sr == "not_found":
                            break
                        if sr == "error":
                            # Unlike "not_found" (which correctly means "no
                            # more shards"), "error" here means a shard that
                            # genuinely exists could not be fetched — treat
                            # it the same as an indexed failure, not a
                            # skippable warning.
                            failed.append(shard_r)
        ...
        r = _download_file_from_github(INSTALLER_MANIFEST_REL, workspace / INSTALLER_MANIFEST_REL)
        if r == "error":
            failed.append(INSTALLER_MANIFEST_REL)
        if r == "ok":
            for idx in range(2, 50):
...
def _download_file_from_github(
    ...
    # Retry with backoff on transient network errors/timeouts (large manifest
    # shards can be several MB and a single flaky connection shouldn't cause
    # them to be silently skipped — a skipped-but-existing shard is what
    # corrupts the local workspace's view of what's already been split on
    # GitHub, which is what produces bad re-splits later).
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        if attempt < max_attempts:
            time.sleep(min(2 ** attempt, 20))   # 2s, 4s, 8s, ...

    if data is None:
        warn(f"Could not download {repo_rel_path} after {max_attempts} attempt(s): {last_exc}")
        return "error"
...
def save_manifest(m: Dict, repo: Path, cat: str = "") -> Path:
    ...
    warn(f"Manifest shard for [bold]{escape(cat)}[/bold] would exceed {fmt_size(MANIFEST_SIZE_LIMIT)} — splitting into new shard(s).")
    try:
    ...
    SAFETY_TARGET  = int(MANIFEST_SIZE_LIMIT * 0.90)         # pack conservatively; exact-verify after
    ...
    def _entry_marginal_cost(entry: Dict) -> int:
        ...
        # before the exact true-up pass catches it.
        probe = _skeleton(start_idx)
        probe["drivers"] = [entry]
        return _exact_size(probe) - empty_overhead
    ...
    while i < len(shards):
        while (len(shards[i]["drivers"]) > 1
               and _exact_size(shards[i]) > MANIFEST_SIZE_LIMIT):
            overflow = shards[i]["drivers"].pop()
            if i + 1 >= len(shards):
                idx += 1
                shards.append(_skeleton(idx))
            shards[i + 1]["drivers"].insert(0, overflow)
        if len(shards[i]["drivers"]) == 1 and _exact_size(shards[i]) > MANIFEST_SIZE_LIMIT:
    ...
    while _exact_size(shards[-1]) > MANIFEST_SIZE_LIMIT and shards[-1]["drivers"]:
        idx += 1
        tail = _skeleton(idx)
        tail["version_history"]       = shards[-1]["version_history"]
        shards[-1]["version_history"] = {}
        while (shards[-1]["drivers"]
               and _exact_size(shards[-1]) > MANIFEST_SIZE_LIMIT):
            tail["drivers"].insert(0, shards[-1]["drivers"].pop())
        shards.append(tail)
        for n, s in enumerate(shards):
            s["shard_index"] = start_idx + n

    if _exact_size(shards[-1]) > MANIFEST_SIZE_LIMIT:
    ...
    for shard in shards:
        ...
        ok(f"New manifest shard: [bold]{escape(path.name)}[/bold]  "
           f"({len(shard['drivers'])} driver(s), {fmt_size(path.stat().st_size)})")
        last_path = path
...
def _installer_manifest_shard_paths(repo: Path) -> List[Path]:
    ...
    idx = 2
    while True:
        p = repo / _installer_shard_name(idx)
        if p.exists():
            paths.append(p); idx += 1
        else:
            break
...
def load_all_installers_combined(repo: Path) -> Dict:
...
def save_installer_manifest(m: Dict, repo: Path) -> Path:
    ...
    warn(f"Installer manifest shard would exceed {fmt_size(MANIFEST_SIZE_LIMIT)} — splitting into new shard(s).")
    try:
        sealed_data = json.loads(active_path.read_text(encoding="utf-8"))
    ...
    SAFETY_TARGET  = int(MANIFEST_SIZE_LIMIT * 0.90)
    ...
    while i < len(shards):
        ...
        if len(shards[i]["installers"]) == 1 and _exact_size(shards[i]) > MANIFEST_SIZE_LIMIT:
    ...
    for shard in shards:
        ...
        ok(f"New installer manifest shard: [bold]{escape(path.name)}[/bold]  "
           f"({len(shard['installers'])} installer(s), {fmt_size(path.stat().st_size)})")
        last_path = path
    ...
    try:
        ...
        if exe_files:
            ...
            if inst_entries:
                ...
                existing_ids = {e["id"] for e in load_all_installers_combined(repo_dir).get("installers", [])}
