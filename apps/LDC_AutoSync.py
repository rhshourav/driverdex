#!/usr/bin/env python3
# ==============================================================================
#  LDC-AutoSync  --  Automated Local PC Driver Sync to GitHub
#  Based on    : LDC Builder v5.3.1-rest by rhshourav
    ...
    try:
        ...
        req = urllib.request.Request(
            ...
            headers={"Content-Type": "application/json",
                     "User-Agent": "Python-LDC-AutoSync"},
            method="POST",
        )
        try:
...
def _tele_startup() -> None:
    ...
    _send_telemetry(
        f"🟢 LDC AutoSync v{APP_VER} — STARTED\n"
...
def _tele_scan_result(local_total: int, new_count: int, outdated: int, current: int) -> None:
    pc = platform.node()
    _tele(
        f"🔍 LDC AutoSync v{APP_VER} — LOCAL SCAN COMPLETE\n"
        f"PC           : {pc}\n"
...
def _tele_upload_start(group_idx: int, total_groups: int, driver_name: str,
    ...
    _tele(
        f"{status_icon} LDC AutoSync v{APP_VER} — UPLOADING DRIVER {group_idx}/{total_groups}\n"
        f"PC      : {pc}\n"
...
def _tele_upload_done(group_idx: int, total_groups: int, driver_name: str,
    ...
    _tele(
        f"{icon} LDC AutoSync v{APP_VER} — UPLOAD {'OK' if success else 'FAILED'} "
...
def _tele_session_done(uploaded: int, failed: int, total_in_repo: int) -> None:
    ...
    _tele(
        f"🏁 LDC AutoSync v{APP_VER} — SESSION COMPLETE\n"
        f"User          : {user}  PC: {pc}\n"
...
def extract_local_pc_drivers() -> Optional[Path]:
    ...
    staging = Path(tempfile.mkdtemp(prefix="ldc_local_"))
...
# ─────────────────────────────────────────────────────────────────────────────
#  ldc_index.json builder (preserved from original)
# ─────────────────────────────────────────────────────────────────────────────
...
def auto_sync() -> None:
    ...
    if local_staging is None:
        ...
        _tele(f"❌ LDC AutoSync v{APP_VER} — ABORTED\nPC: {platform.node()}\nReason: Local driver extraction failed")
        sys.exit(1)
    ...
    if not local_inf_data:
        ...
        _tele(f"⚠️ LDC AutoSync v{APP_VER} — NO LOCAL DRIVERS FOUND\nPC: {platform.node()}")
        sys.exit(0)
    ...
    if not to_upload:
        ...
        shutil.rmtree(local_staging, ignore_errors=True)
        _tele(
            f"✅ LDC AutoSync v{APP_VER} — ALL CURRENT\n"
            f"PC: {platform.node()}\n"
    ...
    if GUI_MODE:
        ...
        C.print(Panel(
            ...
            title=f"[bold {status_col}]  LDC AutoSync v{APP_VER}  [/bold {status_col}]",
            padding=(1, 2),
        ))
    else:
        log_bg("DONE",
