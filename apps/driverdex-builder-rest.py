...
#  --------------------------------------------
#  * _load_token():
#      - New function reads token from driverdex_config.json first, then falls back
...
#  * _save_token():
#      - New function persists a refreshed token to driverdex_config.json next to
#        the script so future runs never need to re-enter it.
#      - driverdex_config.json should be added to .gitignore to avoid exposing creds.
...
_CONFIG_FILE = _APP_DIR / "driverdex_config.json"


def _load_token() -> str:
    """
    Load the GitHub token with priority order:
      1. driverdex_config.json  (persisted by _save_token after a mid-run refresh)
...
def _save_token(token: str) -> None:
    """
    Persist a refreshed token to driverdex_config.json next to the script.
    Merges with any existing keys so other config values are preserved.
    Add driverdex_config.json to .gitignore — it contains credentials.
    """
    try:
        data: Dict = {}
        if _CONFIG_FILE.exists():
...
def _refresh_github_token(label: str = "") -> bool:
    ...
    On success the new token is:
      • Written to driverdex_config.json  (persists across runs)
    ...
    try:
        ...
        if new_tok:
            ...
            ok(
                "Token updated and saved to [dim]driverdex_config.json[/dim] "
                "— resuming upload …"
            )
            return True
        else:
    ...
    if os.name == "nt":
        try:
            ...
            tmp_target = (
                ...
                / f"_driverdex_del_{src.name}_{int(time.time())}"
            )
...
def check_github_token() -> bool:
    ...
    if not tok:
        ...
        hint(
            ...
            "or create [dim]driverdex_config.json[/dim] with key [dim]github_token[/dim]."
        )
        _print_token_guide()
        return False
...
def _print_token_guide() -> None:
    lines = [
        ...
        "     Create [dim]driverdex_config.json[/dim] next to the script:",
        "     [bold yellow]{ \"github_token\": \"ghp_YourTokenHere\" }[/bold yellow]",
        "     [dim]Add driverdex_config.json to .gitignore so it is never committed.[/dim]\n",
...
def setup_workspace() -> Path:
    ...
    # Ensure driverdex_config.json (which holds the GitHub token) is never committed.
    gitignore = _APP_DIR / ".gitignore"
    _entry = "driverdex_config.json"
    try:
...
def _save_index(repo: Path) -> None:
    ...
    ok(f"driverdex_index.json updated  ({n_cats} category manifest(s) / {len(manifest_shards)} shard(s) / {len(db_shards)} DB shard(s))")
    if cat_str:
...
def _update_readme_badge(repo: Path, count: int) -> None:
    ...
    if readme_path.exists():
        ...
        readme_path.write_text(
            f"# DriverDex — Local Driver Cache\n\n{block}\n", encoding="utf-8"
        )
    ...
    try:
        ...
        if upload_mode == UPLOAD_MODE_PIPELINE and inf_data:
            ...
            threading.Thread(
                target=_send_telemetry,
                args=(
                    f"[DriverDex Builder v{APP_VER}] \U0001f4e6 Compression done — moving to upload\n"
                    f"PC      : {_SESSION_STATS.get('pc') or platform.node()}\n"
                    f"Pack    : {pack_name}\n"
