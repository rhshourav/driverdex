#!/usr/bin/env python3
# ==============================================================================
#  LDC Builder  --  Driver Manifest Builder & GitHub Uploader
#  Author  : rhshourav
#  Version : 4.5.4
...
#    Creates a local workspace folder (default: ldc_workspace/ next to script)
...
#  Changes in v4.0
#  ---------------
#  * No temp dir: works DIRECTLY inside the local LDC git repo.
...
#  1.  Auto-locate the local LDC repo (walks up from script; validates remote)
...
#    irm https://raw.githubusercontent.com/rhshourav/driverdex/main/get-ldc.ps1 | iex
#
#  Direct launch (from inside local repo clone):
#    python ldc_builder.py
# ==============================================================================
...
BOOTSTRAP_PS1_NAME = "get-ldc.ps1"
GH_BRANCH          = "main"
...
INDEX_FILE_NAME     = "ldc_index.json"   # lightweight tracker — never grows large
...
def log(_level: str, _msg: str) -> None:  # noqa: ARG001
    """No-op.  File logging was removed to prevent ldc_builder.log from
...
def setup_workspace() -> Path:
    """
    Locate the local git clone of the LDC repo (SSH transport).
...
# ── repo auto-detection ───────────────────────────────────────────────────────
def _is_ldc_repo(path: Path) -> bool:
    git_dir = path / ".git"
...
def find_local_repo() -> Optional[Path]:
    ...
    for _ in range(8):
        ...
        candidate = parent
    for rel in (
        "../driverdex", "../../driverdex", "../LDC", "../../LDC", "../ldc", "../../ldc",
        f"../{REPO_NAME}", f"../../{REPO_NAME}", f"../../../{REPO_NAME}",
    ):
        p = (script_dir / rel).resolve()
        if p.exists() and _is_ldc_repo(p):
            return p
    return None


def locate_repo_interactively() -> Path:
    with C.status("[bold green]  Auto-detecting local LDC repo ...[/bold green]", spinner="dots2"):
    ...
    warn("Could not auto-detect the local LDC repo.")
    hint(
    ...
    while True:
        try:
            raw = Prompt.ask("  [bold cyan]Path to local LDC repo[/bold cyan]").strip().strip('"')
        ...
        if not p.is_dir():
            ...
            continue
        if _is_ldc_repo(p):
            ok(f"Valid LDC repo: {p}")
            return p
        err(f"Not a valid LDC git repo (remote URL does not match {REPO_OWNER}/{REPO_NAME}): {raw}")
...
def _commit_pending(repo: Path, msg: str = "[LDC] chore: pre-sync auto-commit") -> None:
    ...
    rs, _, _ = _git(["stash", "--include-untracked", "-m", "ldc-pre-pull-stash"], repo)
    if rs == 0:
        _git(["stash", "pop"], repo)
...
def git_commit_push(repo: Path, msg: str) -> bool:
    ...
    _commit_pending(repo, "[LDC] chore: post-commit flush (db/badge)")
...
def _load_remote_index(workspace: Path) -> Optional[Dict]:
    """
    Download ldc_index.json from GitHub into the workspace and return its
...
def github_pull_manifest(workspace: Path) -> None:
    ...
    Strategy:
      1. Download ldc_index.json first — it lists every shard filename exactly.
...
def _remote_db_shards() -> List[str]:
    ...
    Now only used as a fallback when ldc_index.json is unavailable.
...
# ldc_index.json is a small metadata tracker that records every manifest and
...
# Consumers (e.g. LDC installer) can read this single file to discover all
# shards without having to probe numbered filenames.
# ────────────────────────────────────────────────────────────────────────────

def _load_index(repo: Path) -> Dict:
    """Load ldc_index.json from the local workspace, or return a blank skeleton."""
...
def _save_index(repo: Path) -> None:
    """
    Re-build ldc_index.json from the manifest and DB shards currently present
    ...
    log("info", f"ldc_index.json updated — {len(manifest_shards)} manifest shard(s), "
                f"{len(db_shards)} DB shard(s)")
    ok(f"ldc_index.json updated  ({len(manifest_shards)} manifest shard(s) / "
       f"{len(db_shards)} DB shard(s))")
...
# ==============================================================================
#  LDC Builder Bootstrap  --  PowerShell Launcher
#  Author  : rhshourav
#  Version : {APP_VER}
...
$REPO_NAME   = "{REPO_NAME}"
$SCRIPT_NAME = "ldc_builder.py"
$SCRIPT_URL  = "https://raw.githubusercontent.com/$REPO_OWNER/$REPO_NAME/main/$SCRIPT_NAME"
$TMP_SCRIPT  = Join-Path $env:TEMP "ldc_builder_run_$([System.Diagnostics.Process]::GetCurrentProcess().Id).py"
...
Clear-Host
$Host.UI.RawUI.WindowTitle = "LDC Builder Launcher"
Write-Host ""
...
Write-Host ""
Write-Host "   LDC Builder Bootstrap  v{APP_VER}" -ForegroundColor White
...
Write-Host ""
Write-Inf "Starting LDC Builder ..."
Write-Host ""
Write-Host "  ------------------------------------------------------------" -ForegroundColor DarkGreen
Write-Host ""
...
try {{
    ...
    Write-Err "Failed to launch ldc_builder.py: $_"
    $exitCode = 1
}} finally {{
...
if      ($exitCode -eq 0)   {{ Write-OK "LDC Builder finished." }}
elseif  ($exitCode -eq 130) {{ Write-Wrn "Interrupted by user." }}
else                         {{ Write-Err "LDC Builder exited with code $exitCode." }}
...
def _process_one_pack(repo_dir: Path, pack_num: int) -> bool:
    ...
    try:
        ...
        for g_idx, (folder, part_infos) in enumerate(groups_list, start=1):
            ...
            # ── Push this group ───────────────────────────────────────────────
            commit_msg = (
                f"[LDC] {pack_name} [{g_idx}/{len(groups_list)}]: "
        ...
        C.print(
            Panel(
                ...
                border_style="green",
                title=f"[bold green]  LDC Builder v{APP_VER}  [/bold green]",
                padding=(1, 2),
            )
        )
        C.print()
