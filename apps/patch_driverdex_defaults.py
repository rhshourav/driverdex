#!/usr/bin/env python3
"""
patch_driverdex_defaults.py
Applies 4 changes to driverdex_builder.py so every interactive prompt
defaults to Yes, speeding up the workflow.

Usage:
    python patch_driverdex_defaults.py                    # patches driverdex_builder.py in CWD
    python patch_driverdex_defaults.py path/to/driverdex_builder.py
"""
...
TARGET = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("driverdex_builder.py")
...
print(f"\n  driverdex_builder.py — {TARGET.resolve()}\n")
